...
public class End2endIT {
	...
	public void enrollUsersSetup(SampleStore sampleStore) throws Exception {
		...
		for (SampleOrg sampleOrg : testSampleOrgs) {
			...
			sampleOrg.setAdmin(admin); // The admin of this org -- //设置管理员身份
			// 获取普通用户
			...
			sampleOrg.addUser(user); // Remember user belongs to this Org //组织内添加用户

			final String sampleOrgName = sampleOrg.getName();// 获取组织的命名域
			...
			sampleOrg.setPeerAdmin(peerOrgAdmin); // A special user that can create channels, join peers and install
													// chaincode //设置节点管理员（一个特殊的用户，可以创建通道，加入节点以及安装链码）
	...
	void runChannel(HFClient client, Channel channel, boolean installChaincode, SampleOrg sampleOrg, int delta) {
		...
		try {
			...
			if (installChaincode) {// a.安装链码
				...
				responses = client.sendInstallProposal(installProposalRequest, peers);// 发送安装链码的交易提案
			...
			if (chaincodeEventListenerHandle != null) {
				...
				channel.unregisterChaincodeEventListener(chaincodeEventListenerHandle);
				// Should be two. One event in chaincode and two notification for each of the
				// two event hubs
	...
	Channel constructChannel(String name, HFClient client, SampleOrg sampleOrg) throws Exception {
		...
		// 获取peer的admin用户，实际上是crytogen根据crypto-config.yaml配置的默认admin账户
...
//        boolean doPeerEventing = !testConfig.isRunningAgainstFabric10() && FOO_CHANNEL_NAME.equals(name);
		...
		// 创建一个channel，这里使用的是Configtxgen 生成的“通道配置”文件
		...
		ChannelConfiguration channelConfiguration = new ChannelConfiguration(new File(TEST_FIXTURES_PATH
				+ "/sdkintegration/e2e-2Orgs/" + TestConfig.FAB_CONFIG_GEN_VERS + "/" + name + ".tx"));
		...
		// client.newPeer创建peer节点，然后joinPeer加入通道channel中
		...
		for (String peerName : sampleOrg.getPeerNames()) {
			...
			Properties peerProperties = testConfig.getPeerProperties(peerName); // test properties for peer.. if
																				// any.//获取节点属性
